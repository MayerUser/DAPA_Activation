...
#   - Attention softmax: torch | debug | pwl-N  (from m0_udanf.PWLSoftmax/DebugSoftmax)
#   - MLP activation  : torch | poly-N (from m1_poly_act.PolyGelu) | pwl-N (from m0_udanf.PWLGelu)
...
# ---- Imports for PWL (softmax + gelu) ----
try:
    from m0_udanf import PWLSoftmax, DebugSoftmax, PWLGelu
except ImportError:
...
def make_softmax_module(
    ...
    softmax_mode:
      ...
      - 'pwl-<N>'-> load <pwl_root>/<num_samples>/pwl_exp_sm_gpt2_<N>seg.json
                    where pwl_root is dst_pwl (dwmse) or dst_pwl_mse (mse)
    """
    ...
    if softmax_mode.startswith("pwl-"):
        ...
        return PWLSoftmax(str(json_path))  # prints/diagnostics configured inside m0_udanf if enabled
...
def inject_act_into_gpt2_mlp(model: nn.Module, act_mod: Optional[nn.Module], device: torch.device):
    """
    Replace GELU activations in GPT-2 MLP with the provided module (PolyGelu/PWLGelu).
    We replace:
      - torch.nn.GELU
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="torch",
        help="MLP activation: 'torch' | 'poly-N' (m1_poly_act.PolyGelu) | 'pwl-N' (m0_udanf.PWLGelu)",
    )
    ...
    # NEW: which PWL set to use (matches t0_make_pwl.py)
    parser.add_argument(
        "--loss",
        type=str,
        default="dwmse",
